...
from analyses.middle_aged import (
    ...
    Q71, Q72, Q73, REQUIRED_HEADERS, MiddleAgedAnalysis,
)
...
class MiddleAgedExcelIntegrationTests(unittest.TestCase):
    def _valid_answers(self, name="正常案例"):
        ...
        values.update({
            ...
            Q21_SOURCE:"很好", Q22_SOURCE:"很好", Q4:"沒有任何影響/我沒有任何疾病",
    ...
    def test_age_column_full_workflow_and_styles(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            lowest = self._valid_answers("低分案例"); lowest.update({"年齡":55,Q1:"分數:0",Q21_SOURCE:"很不好",Q22_SOURCE:"很不好",Q4:"自己覺得完全不能工作",Q5:"100-365天",Q6:"不太可能",Q71:"從不",Q72:"從不",Q73:"從不",DISEASE_HEADERS[0]:"背部",DISEASE_HEADERS[1]:"類風濕性關節炎",DISEASE_HEADERS[2]:"高血壓",DISEASE_HEADERS[3]:"肺結核",DISEASE_HEADERS[4]:"精神疾病或嚴重心理健康問題（如嚴重憂鬱症、心理困擾"}); records.append(lowest)
            missing = self._valid_answers("部分答案缺失"); missing.update({"年齡":60,Q21_SOURCE:""}); records.append(missing)
            no_disease = self._valid_answers("完全沒有疾病案例"); no_disease["年齡"] = 45; records.append(no_disease)
            headers = [*REQUIRED_HEADERS, "年齡"]
    ...
    def test_roc_dates_and_invalid_date_continue(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            headers=[*REQUIRED_HEADERS,"出生日期","體檢日期"]
